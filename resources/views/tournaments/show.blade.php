<x-layout :image="$image" :imageFondo="$imageFondo" :socialmedias="$socialmedias" :sponsors="$sponsors" :teams="$teams">
    ...
    <x-main :tweets="$tweets" :sponsors="$sponsors" :matchesBefore="$matchesBefore" :matchesAfter="$matchesAfter">
        <x-card>
                ...
                @if(count($matches) > 0)
                    @foreach($matches as $match)
                        <section class="flex">
                            ...
                            <p>Pio Pio eSports club vs {{$match->rival}} {{$match->best_of}} @isset($match->result) @if($match->result == 'Victoria') ✅ @elseif($match->result == 'Empate') 🟰 @elseif($match->result == 'Derrota') ❌ @else 🔜 @endif @endisset</p>
                            @if(Auth::check() && in_array(Auth::user()-> role, ['admin', 'superadmin']))
                                <form action="{{route('matches.edit', $match->id)}}">
            ...
            @endisset
            @if(Auth::check() && in_array(Auth::user()-> role, ['admin', 'superadmin']))
                <form action="{{route('matches.create')}}">
            ...
            @endif
            @isset($tournament->result)
                <h1>Posicion actual: {{$tournament->result}}</h1>
            @endisset
