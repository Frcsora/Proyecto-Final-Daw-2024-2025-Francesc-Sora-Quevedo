...
<div class="w-48 sm:w-64 md:w-[80%] gap-3 rounded-lg before:rounded-lg tarjeta p-2 flex flex-col justify-around items-center relative before:absolute before:opacity-80 before:bg-[wheat] before:p-2 before:w-full before:h-full before:content[''] before:z-[-1]">
    ...
    <img class="rounded-lg w-[80%] " src="{{$img}}" alt="{{$name}}">
    <p class="bg-white text-lg p-1">{{$sinopsis}}</p>
    @if(count($tags) > 0)
        <p>
            Etiquetas:
            @foreach($tags as $tag)
