            ...
            @isset($tournaments)
                ...
                @foreach($tournaments as $tournament)
                    <section class="flex flex-wrap gap-2">
                        <p><a href="{{route('tournaments.show', $tournament->id)}}">{{$tournament->event}}: {{$tournament->name}}.</a> Fecha de inicio: {{\Carbon\Carbon::parse($tournament->date)->format('d/m/Y')}} {{$tournament->time}} @isset($tournament->result) Posición: {{$tournament->result}}@endisset</p>
