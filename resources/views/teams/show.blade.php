            ...
            @isset($tournaments)
                ...
                @foreach($tournaments as $tournament)
                    <section class="flex flex-wrap gap-2">
                        <p >{{$tournament->event}}: {{$tournament->name}}. Fecha de inicio: {{\Carbon\Carbon::parse($tournament->date)->format('d/m/Y')}} {{$tournament->time}} @isset($tournament->result) Posición: {{$tournament->result}} @endif</p>
